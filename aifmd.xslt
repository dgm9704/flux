	...
	<xsl:template match="AIFMRecordInfo">
		...
		<xsl:variable name="CAM-002">
			<xsl:choose>
				...
				<xsl:when test="$periodtype='Q1' or $periodtype='H1' or $periodtype='Y1'">
					<xsl:if test="not($month='01')">true</xsl:if>
				</xsl:when>
				<xsl:when test="$periodtype='Q2'">
					<xsl:if test="not($month='04')">true</xsl:if>
				</xsl:when>
				<xsl:when test="$periodtype='Q3' or $periodtype='H2'">
					<xsl:if test="not($month='07')">true</xsl:if>
				</xsl:when>
				<xsl:when test="$periodtype='Q4'">
					<xsl:if test="not($month='10')">true</xsl:if>
				</xsl:when>
			</xsl:choose>
		</xsl:variable>
